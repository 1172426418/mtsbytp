                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="header text-center">
                                <h4 class="title">友情链接设置</h4>
                                <p class="category" style="color:orange"></p>
                                <br/>
                                <input id="infor_insert" type="button" value="添加新链接" onclick="window.location.href='/xmcy/admin.php/Blogroll/add_interlink.html'" style="width:140px;height:35px;font-size:16px;float:left;background:white;border:1px solid #888">
                            </div>
                            <div class="content table-responsive table-full-width">
                                <table class="table table-bigboy">
                                    <thead>
                                        <tr>
                                            <th class="text-center">logo图</th>
                                    ...
                                    <tbody>
                                    <?php if(is_array($urls)): foreach($urls as $key=>$v): ?><tr align="center">
                                            <td style="width:10%;padding:4px 0;">
                                                <img src="/xmcy/<?php echo ($v["path"]); ?>" style="width:100px;height:70px;">
                                            </td>
                                            ...
                                            <td class="select-state" data-id="<?php echo ($v["id"]); ?>">
                                                <?php if($v['state']==1): ?><img data-id="<?php echo ($v["id"]); ?>" src="/xmcy/public/img/effect/checked.png">
                                                    <?php else: ?>
                                                    <img data-id="<?php echo ($v["id"]); ?>" src="/xmcy/public/img/effect/unchecked.png"><?php endif; ?>
                                            </td>
                                            <td class="effect">
                                                <img class="infor_update" data-id="<?php echo ($v["id"]); ?>" src="/xmcy/public/img/effect/asadaf.jpg" alt="改变状态" title="改变状态" style="width:21px;height:22px;margin-right:9px;">
                                                <img src="/xmcy/public/img/effect/update.png" onclick="window.location.href='/xmcy/admin.php/Blogroll/edit/id/<?php echo ($v["id"]); ?>.html'" alt="编辑" title="编辑">&nbsp;&nbsp;
                                                <img class="blog_delete" data-state="<?php echo ($v["state"]); ?>" data-id="<?php echo ($v["id"]); ?>" src="/xmcy/public/img/effect/delete.png" alt="删除" title="删除">
                                            </td>
...
<script>
    ...
    $('.infor_update').click(function(){
        ...
        $.ajax({
            type:'post',
            url:"/xmcy/admin.php/Blogroll/blog_update",
            data:'id='+id,
            success:function(data){
                if(data=='dowm'){
                    $('.select-state img[data-id='+id+']').attr('src','/xmcy/public/img/effect/unchecked.png');
                    $('.blog_delete[data-id='+id+']').attr('data-state',0);
                }else{
                    $('.select-state img[data-id='+id+']').attr('src','/xmcy/public/img/effect/checked.png');
                    $('.blog_delete[data-id='+id+']').attr('data-state',1);
                }
            }
        })
    })
    ...
    $('.blog_delete').click(function(){
        ...
        if(state==1){
            One_button('展示状态下无法直接删除，请先停止展示！');
        }else{
            Double_button('确定删除该链接吗？','/xmcy/admin.php/Blogroll/delete/id/'+id+'.html');
        }
    })
</script>
