...
class IndustryController extends BaseController {
    ...
    {
        if($_GET['ind']=='index'){
            $_GET['id']=$_SESSION['ind']['nowpage'];
            $_POST=$_SESSION['ind']['post'];
        }
        ...
        if($_POST){
            ...
            $arr=M()->query($sql);
            $_SESSION['ind']['post']=$_POST;           //SESSION储存搜索条件
            $_SESSION['ind']['nowpage']=$nowpage;      //SESSION储存当前页
            $this->assign('post',$_POST);
        }else{
            $num = M('production')->where('pid=3 && is_delete=0')->count();
            $pages = ($num==0)?1:ceil($num/$length);  //总页数
            if($_GET['id']>$pages){
                $nowpage = $pages;
            }
            $offset = ($nowpage-1)*$length;                  //偏移量
            $limit = $offset.','.$length;
            $sql='select p.id,p.path,p.title,n.menu,p.author,p.date,p.state from production as p inner join navigation as n on p.navi_id=n.id where n.pid=3 && p.is_delete=0 limit '.$limit;
            $arr=M()->query($sql);
        }
        foreach($arr as $k=>$v){
            $arr[$k]['date']=date('Y年m月d日 H时i分',$v['date']);
        }
        $navigation=M('navigation')->where('pid=3')->select();
    ...
    public function recycle()
    {
        ...
        if($_POST){
            ...
            $num = M('production')->where('(navi_id=0 || pid=3) && is_delete=1')->count();
            $pages = ($num==0)?1:ceil($num/$length);  //总页数
            if($_GET['id']>$pages){
                $nowpage = $pages;
            }
            $offset = ($nowpage-1)*$length;                  //偏移量
            $limit = $offset.','.$length;
            $sql='select p.id,p.path,p.title,n.menu,p.author,p.date,p.state,p.navi_id from production as p inner join navigation as n on p.navi_id=n.id where (n.pid=3 || p.navi_id=0) && p.is_delete=1 limit '.$limit;
            $arr=M()->query($sql);
        }
        foreach($arr as $k=>$v){
            $arr[$k]['date']=date('Y年m月d日 H时i分',$v['date']);
    ...
    public function insert_form()
    {
        ...
        $upload->savePath='public/upload/industry/'; //保存路径 必须以/结尾
        ...
        if($up_img){
            ...
            header('location:'.__APP__.'/Industry/index.html');
        }else{
            header('location:'.__APP__.'/Industry/insert.html');
        }
    }
    ...
    public function edit_form($id)
    {
        ...
        header('location:'.__APP__.'/Industry/edit/id/'.$id.'/type/'.$_GET['type'].'.html');
    }
    ...
    public function delete($id)
    {
        ...
        header('location:'.__APP__.'/Industry/recycle.html');
    }
    ...
    //ajax上线产品
    public function Industry_up($id)
    {
        $n=M($_POST['type'])->where('state=1 && pid=3')->count();
        if($n<10){
            $arr=array('state'=>1);
    ...
    //ajax下线产品
    public function Industry_down($id)
    {
        $arr=array('state'=>0);
        $num=M($_POST['type'])->where('id='.$id)->save($arr);
        echo $num;
    ...
    public function search($search,$num,$type)
    {
        ...
        if($type=='rec'){
            $sql=substr($sql.' && (n.pid=3 || p.navi_id=0) && p.is_delete='.$num,4);
        }else{
            $sql=substr($sql.' && n.pid=3 && p.is_delete='.$num,4);
        }
    ...
    public function delete_menu($id)
    {
        ...
        header('location:'.__APP__.'/Industry/menu.html');
    }
}
